...
againFile = 'prj_list/scheduling_projects____prj_main/%s/output/根据结果重排订单.xlsx'%(prj_name)        # 延期订单拆分后重排
isAgain = False                                  # 是否订单拆分后第二次排产

dataFile = 'prj_list/scheduling_projects____prj_main/%s/output/项目数据导出.xlsx'%(prj_name)# 排产数据文件
resultFile = 'prj_list/scheduling_projects____prj_main/%s/output/智能排产结果.xlsx'%prj_name # 排产结果文件
...
##################################################################################本地调试-注释开始
# prj_name = "创元泡罩车间试点"
# dataFile = './output/项目数据导出.xlsx'             # 排产数据文件
# againFile = './output/根据结果重排订单.xlsx'        # 延期订单拆分后重排
# isAgain = False                                  # 是否订单拆分后第二次排产
# resultFile = './output/智能排产结果.xlsx'           # 排产结果文件
# startTime = datetime.strptime('2021/8/2 8:00', '%Y/%m/%d %H:%M') # 指定排产开始时间
# maxDays = 100                           # 排产最大时长(单位天）
# period = 1/60                           # 时间最小粒度(单位小时)
# maxValue = int(maxDays*24/period)       # 变量最大值，整型变量加范围，是否速度更快
# solve_max_time = 30                    # 最大运行时长(单位秒）
###################################################################################本地调试-注释结束
...
mdl = CpoModel()                              # 定义求解器
projectState = ProjectState(prj_name)         # 更新项目对象
...
# 读取订单信息
if isAgain:
    orderTable = pd.read_excel(againFile, '客户订单', keep_default_na=False)
else:
    orderTable = pd.read_excel(dataFile, '客户订单', keep_default_na=False)
orderTable['订单优先级'] = orderTable['订单优先级'].astype('category')
hasPriorities = []
for item in priorities:
    if item in orderTable['订单优先级'].cat.categories:
        hasPriorities.append(item)
orderTable['订单优先级'].cat.reorder_categories(hasPriorities, inplace=True)
orderTable = orderTable.sort_values(['订单交期', '订单优先级'])

# # 读取遗留工单信息
# resultTable = pd.read_excel(resultFile, '工单计划', keep_default_na=False)
# resultTable['订单优先级'] = resultTable['订单优先级'].astype('category')
# hasPriorities = []
# for item in priorities:
#     if item in resultTable['订单优先级'].cat.categories:
#         hasPriorities.append(item)
# resultTable['订单优先级'].cat.reorder_categories(hasPriorities, inplace=True)
# resultTable = resultTable.sort_values(['订单交期', '订单优先级'])
...
# # 遗留订单工作
# for row, work in resultTable.iterrows():
#     if work['工单状态'] in ['未开工', '进行中'] and work['已完成数量'] < work['订单数量']:
#         workNo = work['工单编号']
#         area = work['区域']
#         resources = str(work['产线'])
#         orderNo = work['订单号']
#         productShort = work['快捷编号']
#         productNo = work['产品编号']
#         quantity = work['订单数量'] - work['已完成数量']
#         priority = work['订单优先级']
#         parentNo = work['父料号']
#         childNo = work['子料号']
#         partName = work['品名']
#         groupName = work['人员区域']
#         deviceTypes = work['设备类型']
#         deviceNums = work['设备数量']
#         constraintType = work['约束类型']
#         isFixed = work['是否固定'] == '是'
#         fixTime = work['固定时间']

#         # 订单交期
#         deliveryDate = work['订单交期']
#         if isinstance(deliveryDate, Timestamp):
#             deliveryDate = deliveryDate.to_pydatetime() + timedelta(hours=17)
#             deliveryDate = round((deliveryDate-startTime).total_seconds()/(3600*period))
#         elif isinstance(deliveryDate, datetime):
#             deliveryDate = deliveryDate + timedelta(hours=17)
#             deliveryDate = round((deliveryDate-startTime).total_seconds()/(3600*period))
#         elif isinstance(deliveryDate, str):
#             deliveryDate = datetime.strptime(deliveryDate+' 17:00', '%Y/%m/%d %H:%M')
#             deliveryDate = round((deliveryDate-startTime).total_seconds()/(3600*period))

#         # 预计到货时间
#         shortPart = work['缺货料号']
#         availableTime = work['预计到货时间']
#         if isinstance(availableTime, Timestamp):
#             availableTime = availableTime.to_pydatetime() + timedelta(hours=8)
#             availableTime = round((availableTime-startTime).total_seconds()/(3600*period))
#         elif isinstance(availableTime, datetime):
#             availableTime = availableTime + timedelta(hours=8)
#             availableTime = round((availableTime-startTime).total_seconds()/(3600*period))
#         elif isinstance(availableTime, str):
#             availableTime = datetime.strptime(availableTime+' 8:00', '%Y/%m/%d %H:%M')
#             availableTime = round((availableTime-startTime).total_seconds()/(3600*period))

#         # 标准产能
#         capacity = str(work['标准产能']).lower()
#         if '||' in capacity:
#             duration = []
...
#         # 班组长人数
#         try:
#             managerNumber = int(work['班组长人数'])
#         except:
#             managerNumber = 0

#         # 操机手人数
#         try:
#             qualifyNumber = int(work['操机手人数'])
#         except:
#             qualifyNumber = 0
        
#         # 最小排队时间，含非工作时间
#         minMoveTime = work['转移时间下限']
...
#         else:
#             minMoveTime = int(minMoveTime)

#         # 最大排队时间，含非工作时间
#         maxMoveTime = work['转移时间上限']
...
#         else:
#             maxMoveTime = int(maxMoveTime)

#         allWorks[orderNo, childNo, work['订单数量'], deliveryDate] = WorkType(workNo, area, resources, orderNo, productShort, productNo, work['订单数量'], deliveryDate, shortPart, availableTime, priority, parentNo, childNo, partName, capacity, groupName, groupNumber, managerNumber, qualifyNumber, deviceTypes, deviceNums, duration, minMoveTime, maxMoveTime, constraintType, isFixed, fixTime, work['工单状态'], work['分配人员'], work['分配班组长'], work['分配操机手'], work['已完成数量'], 0, 0) 
...
for row1, order in orderTable.iterrows():
    ...
    for row2, process in processTable[processTable['产品编号']==order['产品编号']].iterrows():
        ...
        allWorks[orderNo, childNo, quantity, deliveryDate] = WorkType(workNo, area, resources, orderNo, productShort, productNo, quantity, deliveryDate, shortPart, availableTime, priority, parentNo, childNo, partName, capacity, groupName, groupNumber, managerNumber, qualifyNumber, deviceTypes, deviceNums, duration, minMoveTime, maxMoveTime, constraintType, False, '', '未开工', 0, 0)
...
# 构建顺序约束
for work in allWorks.values():
    if work.parentNo != work.childNo and (work.orderNo, work.parentNo, work.quantity, work.deliveryDate) in allTasks.keys():
        parentWork = allWorks[work.orderNo, work.parentNo, work.quantity, work.deliveryDate]
        childJob = allTasks[work.orderNo, work.childNo, work.quantity, work.deliveryDate]
        parentJob = allTasks[work.orderNo, work.parentNo, work.quantity, work.deliveryDate]
...
# 定义目标函数：最后完成时间最小
objectVars.extend([mdl.end_of(allTasks[key])*0.0001 for key in allWorks if allWorks[key].parentNo == allWorks[key].childNo])
...
if status == 'Optimal' or status == 'Feasible': 
    for row, key in enumerate(allWorks):
        ...
        # 记录数据
        result = WorkType(work.workNo, work.area, resources, work.orderNo, work.productShort, work.productNo, work.quantity, work.deliveryDate, work.shortPart, work.availableTime, work.priority, work.parentNo, work.childNo, work.partName, capacity, work.groupName, groupNumber, work.managerNumber, work.qualifyNumber, work.deviceTypes, work.deviceNums, work.duration, work.minMoveTime, work.maxMoveTime, work.constraintType, work.isFixed, work.fixTime, work.workState, beginIndex, endIndex)
        resultWorks.append(result)
        resourceWorks[resources].append(result)

        # 超期订单拆分
        if work.parentNo == work.childNo and endIndex + work.minMoveTime > work.deliveryDate:
            delayTime = endIndex + work.minMoveTime - work.deliveryDate
            if delayTime > 720:    # 超期较少不用延期
                splitQuantity = round(work.quantity/2)-1
                remainQuantity = work.quantity - splitQuantity
                for row1, order in orderTable.iterrows():
                    # 计算订单交期
                    try:
                        tmpDeliveryDate = order['订单交期']
                        if isinstance(tmpDeliveryDate, Timestamp):
                            tmpDeliveryDate = tmpDeliveryDate.to_pydatetime() + timedelta(hours=17)
                        elif isinstance(tmpDeliveryDate, datetime):
                            tmpDeliveryDate = tmpDeliveryDate + timedelta(hours=17)
                        elif isinstance(tmpDeliveryDate, str):
                            tmpDeliveryDate = datetime.strptime(tmpDeliveryDate+' 17:00', '%Y/%m/%d %H:%M')
                        tmpDeliveryDate = round((tmpDeliveryDate-startTime).total_seconds()/(3600*period))
                    except:
                        tmpDeliveryDate = maxValue
                    # 找到对应订单并拆分
                    if order['订单号'] == work.orderNo and order['产品编号'] == work.productNo and order['订单数量'] == work.quantity and tmpDeliveryDate == work.deliveryDate:
                        remainOrder = pd.Series(order)
                        remainOrder['订单数量'] = remainQuantity
                        orderTable.loc[row1] = remainOrder
                        splitOrder = pd.Series(order)
                        splitOrder['订单数量'] = splitQuantity
                        orderTable.loc[len(orderTable)] = splitOrder
        # 保存拆分订单
        orderTable.to_excel(againFile, '客户订单', index=False)
...
if status == 'Optimal' or status == 'Feasible': 
    ...
    for row, work in enumerate(resultWorks):
        ...
        # Excel颜色变化
        if work.parentNo == work.childNo and work.endTime + work.minMoveTime > work.deliveryDate:
            style = style_red
